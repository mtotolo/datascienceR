require(dplyr)
require(slam)
mydictionary<-readRDS("mydictionary.rds")
bimatrix<-readRDS("bimatrix.rds")
trimatrix<-readRDS("trimatrix.rds")
quadmatrix<-readRDS("quadmatrix.rds")
filler<-function(x) {
      NAs<-sum(is.na(x))
      if (NAs==0) return(x)
      j<-1
      for (i in length(x)-NAs+1:length(x)-1) {
            while (is.na(x[i])) {
                  y<-as.character(mydictionary[j,1])
                  if(!(y %in% x)) x[i]<-y
                  j<-j+1
            }
      }
...
#                                                      which(mydictionary$label==y),])),]) 
      ...
      if (length(i)>0 & length(j)>0) {
            ...
            if (length(indexes)>0  & !all(is.na(indexes)))
            return(fillerfun(as.character(mydictionary[
                  drop_simple_sparse_array(trimatrix[i,j,])$i[indexes],1]),
                  findBest2(x[2])))
      }
      return(findBest2(x[2]))
}
...
#            print("1 in")
            ...
            if (length(indexes>0) & !all(is.na(indexes)))
                  return(fillerfun(as.character(mydictionary[unique(
...
#            print("2 in")
            ...
            if (length(indexes>0) & !all(is.na(indexes))) 
                  return(filler(as.character(mydictionary[unique(
